...
class Game {
    ...
    var playerGenerator: StandartPlayerGenerator
    var mapRenderer: StandartMapRender

    init(mapGenerator: StandartMapGenerator, playerGenerator: StandartPlayerGenerator, mapRenderer: StandartMapRender) {
    ...
    func run() {
       ...
       for i in 1...totalPlayers {
        ...
        players.append(playerGenerator.generatePlayer(name: "Player #\(i)") as! StandartPlayer)
       }
       

        let map = mapGenerator.generate(players: players)
        mapGenerator.placePlayersRandomlyOnMap(players: players, maze: map.maze as! [[StandartMapTile]])
        // 1. Избор на брой играчи. Минимум 2 броя.
        ...
        while activePlayers(allPlayers: players).count > 1  {
            if var currentPlayer:StandartPlayer = players[currentPlayerIndex], currentPlayer.isAlive {
                let playerNumber = currentPlayerIndex + 1
                print("Сега е на ход играч №\(playerNumber) - \(currentPlayer.name) \(currentPlayer.avatar)")
                    ...
                    if let command = readLine(as: String.self) {
                        ...
                        if let move = availableMoves.first(where: { (move) -> Bool in
                            ...
                            case "map":
                                ...
                                print("available places: \(mapGenerator.checkAvailablePlacesToSpawn(maze: map.maze))")
                            case "seppuku":
